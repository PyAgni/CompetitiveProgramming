...
int main(){
	...
	tin{
		...
		read(a,b,c,d,x,y,x1,y1,x2,y2);

		// if(x-a<x1 || x+b>x2 || y+d>y2 || y-c<y1)
		// 	cout<<"NO\n";
		// if(abs(a-b)>abs(x2-x1) || abs(c-d)>abs(y2-y1))
		// 	cout<<"NO\n";
		// // else
		if(x-(a-b)>=x1 && (x2>x1 || a+b==0) && x-(a-b)<=x2 && y-(c-d)>=y1 && (y2>y1 || c+d==0) && y-(c-d)<=y2)
			cout<<"Yes\n";
		else
			cout<<"No\n";
